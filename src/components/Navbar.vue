        ...
        <div class="flex flex-wrap items-center justify-between px-9 md:py-4 bg-white/[.9] dark:bg-slate-900 backdrop-blur-sm transition-shadow duration-300" :class="{ 'shadow-lg dark:bg-slate-900/[.8]': !hideNavShadow }">
            ...
            <router-link to="/#landing-page">
                <div :class="['transition-all motion-reduce:transition-none duration-500', showTransition ? 'opacity-1 blur-0' : 'opacity-0 blur-sm']">
                    ...
                    <!-- Custom Logo -->
                    <div v-else>
                        <div class="flex items-center space-x-2 py-4 md:py-1 drop-shadow-lg bg-gradient-to-r from-[#DD1EEB] to-[#5e17eb] bg-clip-text text-transparent">
                            <p>{</p>
                            <p class="text-3xl md:text-5xl" id="custom-logo">U.S</p>
                            <p>}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="hidden w-full xl:block xl:w-auto items-center" id="navbar-default">
                <ul class="font-medium flex flex-col p-4 xl:p-0 mt-4 xl:flex-row xl:space-x-8 xl:mt-0">
                    ...
                    <!-- <li :class="['transition-all motion-reduce:transition-none duration-500 delay-[150ms]', showTransition ? 'translate-y-0 opacity-1' : '-translate-y-4 opacity-0']">
                        <router-link class="block py-2 pl-3 pr-4 hover:text-link-color" to="/#work-section">Skills</router-link>
                    </li> -->
                    ...
                    <router-link to="/#contact-section">
                        <button
                        class="block py-2 px-6 bg-transparent border border-button-color shadow-sm shadow-button-color text-button-color 
                                transition ease-in-out hover:bg-button-color hover:text-white hover:dark:text-slate-300 
                                focus:bg-button-color focus:text-white active:bg-button-color active:text-white duration-300 rounded-full"
                        >
                        Contact
                        </button>
                    </router-link>
        ...
        <NavTransition>
            <div v-show="dropDownActive" class="block w-full absolute xl:hidden px-9 pb-4 bg-white/[.9] dark:bg-slate-900/[.8] backdrop-blur-sm">
                <ul>
                    ...
                    <li>
                        <router-link to="/#contact-section">
                            <button class="block w-full text-center bg-button-color py-2 px-6 mt-5 shadow-sm shadow-button-color focus:bg-button-color focus:text-white active:bg-button-color active:text-white text-white">Contact</button>
                            </router-link>
                    </li>
                    <li>
